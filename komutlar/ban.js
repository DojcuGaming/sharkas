...
let modlog = await db.fetch(`mlogg_${message.guild.id}`) 
  if (!modlog) return message.channel.send("<a:dikkat:739388861469163521> Ayarlı Bir Ban Log Kanalı Yok! Ayarlamak İçin \`s!ban-log #kanal\` !")   
    if(message.channel.type == "dm")  return;
  if(message.channel.type !== "text") return;
    if (!message.member.hasPermission("BAN_MEMBERS")) return message.channel.send("<a:dikkat:739388861469163521>Bu komudu kullanabilmek için `Ban` yetkisine sahip olmanız gerek.");
    let reason = args.slice(1).join(' ')
    if (!args[0]) return message.channel.send("<a:dikkat:739388861469163521> Yasaklamak istediğiniz kullanıcıyı etiketleyiniz.")
    let user = message.mentions.users.first() || bot.users.cache.get(args[0])
    if (!user) return message.channel.send(`<a:dikkat:739388861469163521> Etiketlediğin kullanıcıyı sunucuda bulamadım.`)
    let member = message.guild.member(user)
    if (!member) return message.channel.send(`<a:dikkat:739388861469163521> Etiketlediğin kullanıcıyı sunucuda bulamadım.`)
